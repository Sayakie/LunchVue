...
export class LunchVue {
  ...
  public async fetch(domain: string, code: string, month: number): Promise<any> {
    ...
    const payload: request.CoreOptions & (request.UriOptions | request.UrlOptions) = { ...this.payload, uri }

    const response = await this.request(payload)
    const feedData: any[] = this.getMeals(response)
  ...
  private normalizationHTML(body: string) {
    // 영양소 성분이 없는 식단도 줄 바꿈을 해주기 위해
    // <br> 엘리먼트가 필터링되기 전에 영양소 성분처럼 '흉내내어(HACKED)' 대체합니다.
    const breakToken = '1.'
